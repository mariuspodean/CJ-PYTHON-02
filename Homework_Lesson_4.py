...
# 1). two dicts that group all data by country for each year
...
# 2). - one dict that groups all data by year for Germany
...
# 3). - one dict that groups all data by country and year, by using year in the key together with the country name
...
# 4). - starting from the previous health_index dict, display only the data where the health_index > 5
...
# 5). - starting from the previous health_index dict, display only the data where the health_index > 5 and sex is ‘F’
...
# 6). - starting from the previous health_index dict, create a for loop to print the health_index
