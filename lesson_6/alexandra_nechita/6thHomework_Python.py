...
#Varianta2 - using description list, multiple for loops

#     for country, coverage in raw_data:
#         for index in reversed(coverage):
#             for year in reversed(description[1]):
#                 dataset_dict[country].append ({'year':year, 'coverage':index})
#     return dataset_dict

#Varianta3 - using description list, only 2 for loops

    for country, coverage in raw_data:
        year_index=len(description[1])-1
        for index in reversed(coverage):
            dataset_dict[country].append ({'year':description[1][year_index], 'coverage':index})
...
#         if coverage.isdigit():
#             final_coverage=int(coverage)
#             suma=suma+final_coverage
#             elements=elements+1
#     return float(suma/elements)

#Varianta 2 - where we extract the numeric data from the non-numeric data for coverage
...
def perform_average(coverage_list):
    ...
    for country_or_year, coverage in coverage_list:
        ...
        for letter in coverage:
            if letter.isdigit():
                only_numeric_string +=letter
        if only_numeric_string.isdigit():
            suma=suma+int(only_numeric_string)
            elements=elements+1
    if elements==0:
        return 0
    else:
