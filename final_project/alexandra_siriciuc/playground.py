...
kid1 = Kid('Siriciuc Anisia', 'F', '06-07-2018', '0755955363', 'Siriciuc Alexandra')
...
print (f'For the Teacher {teacher1.get_name()} the annual salary is {teacher1.get_annual_salary(teacher1.get_salary())}')
...
menu_dict2 = {
        'Monday' : {
                    'breakfast' : ['mini mushroom omelettes or breakfast cups', 'strawberries and banana slices'],
                    'lunch' : ['cream of parsnip and carrot soup', 'raw vegetables (carrots, celery sticks, red pepper slices) with salad dressing for dipping'],
                    'snack' : ['apple slices with nut butter or cheddar cheese']
                },
        'Tuesday' : {
                    'breakfast' : ['oatmeal pancakes with apple sauce', 'milk'],
                    'lunch' :['tuna grilled cheese or mini sandwiches on multigrain bread or whole wheat crackers', 'Cucumber slices with dip'],
...
print_group = PrintGroup(group.get_name(), group.get_teacher(), group.get_kids_list())
