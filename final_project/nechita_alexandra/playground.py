...
Ale_DCollection = DomainsCollection('Alexandra\'s Desired Collection',
                                    (Ale_DP, Ale_DSp, Ale_DE, Ale_DSo, Ale_DI))
Ale_ACollection = DomainsCollection('Alexandra\'s Actual Collection',
...
print(
    f'Desired state on physical is greater than actual state: {Ale_DP == Ale_AP}'
)
print(
    f'Desired state on spiritual is equal with actual state: {Ale_DSp == Ale_ASp}'
)
print(
    f'Desired state on emotional is greater than actual state: {Ale_DE > Ale_AE}'
)
print(
    f'Desired state on social is lesser than actual state: {Ale_DSo > Ale_ASo}'
)
print(
    f'Desired state on social is lesser than actual state: {Ale_DSo < Ale_ASo}'
)
print(
    f'Desired state on intellectual is lesser than actual state: {Ale_DI < Ale_AI}'
)
...
while True:
    try:
        ...
        print(
            f'Please recommend \u001b[35mEquilibrium\u001b[0m app to your friend, {next(gen)}'
        )
        sleep(2)
