...
class TestPerson(unittest.TestCase):
    def test_person_object_attribute_initialization(self):
        ...
        assert hasattr(test_person,
                       '_name'), 'Person class is missing _name attribute'
        assert hasattr(test_person,
                       '_gender'), 'Person class is missing _gender attribute'
        assert hasattr(test_person, '_date_of_birth'
                       ), 'Person class is missing _date_of_birth attribute'
        assert hasattr(
            test_person,
            '_phone_number'), 'Person class is missing _phone_number attribute'
    ...
    def test_person_object_attribute_values(self):
        ...
        self.assertEqual(test_person._name,
                         name), 'Person class _name attribute value is wrong'
        self.assertEqual(
            test_person._gender,
            gender), 'Person class _gender attribute value is wrong'
        self.assertEqual(
            test_person._date_of_birth, date_of_birth
        ), 'Person class _date_of_birth attribute value is wrong'
        self.assertEqual(
            test_person._phone_number, phone_number
        ), 'Person class _phone_number attribute value is wrong'

    def test_person_object_age(self):

        p1, p2, p3 = Person('Person1', 'M', '10-02-2015',
                            '0729315687'), Person('Person2', 'F', '10-02-2015',
                                                  '0759335565'), Person(
                                                      'Person3', 'F',
                                                      '10-02-2017',
                                                      '0757835565')
...
class TestKindergarten(unittest.TestCase):
    def test_register_kid(self):
        ...
        self.assertIsInstance(kindergarten,
                              Kindergarten), 'Invalid Kindergarten instance'

        kid = Kid('Siriciuc Anisia', 'F', '06-07-2018', '0755955363',
                  'Siriciuc Alexandra')
        ...
        self.assertEqual((len(group.get_kids_list())),
                         1), 'Kid was not registred'
