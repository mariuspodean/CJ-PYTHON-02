...
class TableOfPlayers(MutableMapping):
    ...
    def __str__(self):
        return (
            f'Table contains the following players: {list(self.players_list.keys())}'
        )
...
def betting_params(type, value):
    options = [{
        'question': 'What number? (1-36) ---> ',
        'error_msg': 'Invalid number'
    }, {
        'question': 'What color? (red/black) --->',
        'error_msg': 'Invalid color'
    }, {
        'question': 'Even or odd? --->',
        'error_msg': 'Invalid option'
    }, {
        'question': 'Low (1-18) or high (19-36) bet? (low/high) ---> ',
        'error_msg': 'Invalid option'
    }]
...
def compare_numbers(player_nr, cpu_nr, type):
    ...
    if type == 1:
        ...
        return True if (cpu_nr[0] % 2 == 0 and player_nr == 'EVEN') or (
            cpu_nr[0] % 2 != 0 and player_nr == 'ODD') else False
    elif type == 4:
        return True if (1 <= cpu_nr[0] < 19 and player_nr == 'LOW') or (
            19 <= cpu_nr[0] < 37 and player_nr == 'HIGH') else False
...
def roulette(players):
    ...
    while play_game != False:
        ...
        for index, player in enumerate(players):
            ...
            while True:
                logging.debug(
                    f'Asking player {player.gamer_name} for a betting option')
                player_option = input(
                    'Which option you choose to bet? (1-4) ---> ')
            ...
            while True:
                logging.debug(
                    f'Asking player {player.gamer_name} for a playing choice')
                player_choice = (input(
                    betting_params(player_option, 'question'))).upper()
            ...
            while True:
                logging.debug(
                    f'Asking player {player.gamer_name} for a betting amount')
                bet_amount = input(
                    f'How much credit you want to bet on this number? You have: {credit} left. ---> '
                )
        ...
        for index, player in enumerate(players):
            ...
            if compare_numbers(player_choice, tables_number, player_option):
                print(
                    f'{player.gamer_name} you WON!!! The number was {tables_number}, your bet was {player_choice} '
                )
                ...
                print(
                    f'{player.gamer_name} you LOST!!! The number was {tables_number}, your bet was {player_choice}'
                )
                credit -= bet_amount
                players[index].player_info['credit'] = credit
            ...
            if no_more_credit(credit):
                ...
                logging.info(
                    f'Asking player {player.gamer_name} if they want to leave the table'
                )
...
class TimeIt:
    ...
    def __enter__(self):
        ...
        self.log_file.write(f'Players credits BEFORE playing roulette: ' +
                            '\n')
        logging.debug(
            'Writing out the players and credits before playing the game')
    ...
    def __exit__(self, type, value, traceback):
        ...
        logging.debug(
            'Writing out the players and credits after playing the game')
        for player in self.players:
            self.log_file.write(
...
def prepare_results_list(players):
    ...
    for index, player in enumerate(players):
        print(
            f'{index+1}: {player.gamer_name}: {player.player_info["credit"]}')


def score_reader(file_name):

    logging.info(
        'printing out the players in the \'scores.txt\' file with the new credits'
    )
