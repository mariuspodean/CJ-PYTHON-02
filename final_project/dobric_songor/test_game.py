...
class TestPlayer(unittest.TestCase):
    def test_player_object_attribute_initialization(self):
        ...
        assert hasattr(test_player,
                       'player_info'), 'Player has no informations'
    ...
    def test_players_get_name_method_returns_gamer_name(self):
        ...
        assert test_player.get_name(
        ) == test_player.gamer_name, 'This is not the players gamer_name'

    def test_players_comparing_checks_players_credits(self):

        player1 = application.Players('Python', {
            'name': 'py',
            'age': 10,
            'credit': 100
        })
        player2 = application.Players('Pygame', {
            'name': 'game',
            'age': 20,
            'credit': 100
        })
        ...
        self.assertEqual(
            result, player1["credit"] ==
            player2["credit"]), 'Not the players credits are being verified'
...
class Roulette(unittest.TestCase):
    def test_generated_number_is_correct(self):
        ...
        self.assertIn(generate_random_color,
                      needed_colors), 'Invalid color generated'
