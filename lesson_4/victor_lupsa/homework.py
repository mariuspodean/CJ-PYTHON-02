...
def make_dic(description, raw_data):
    dic= {}
    entry_list = []
    for x in raw_data:
        i = 0
        for y in description[1]:
            if x[1][i] != ': ':
                new_entry = {'year': y, 'coverage': x[1][i]}
...
dataset = make_dic(description, raw_data)
...
def get_country_data(data_set,country):
    dic ={}
    the_list=[]
    for x in data_set[country]:
        temp = (x['year'],x['coverage']
        )
        the_list.append(temp)
    dic[country] = the_list
    return dic

def get_average_country(country_data):
    divider = len(country_data)
    suma = 0
...
get_year_data(dataset,'2014 ')
data_set_year = get_year_data(dataset,'2016 ')
...
country_data = get_country_data(dataset,'TR')
print(get_average_country(country_data['TR']))
