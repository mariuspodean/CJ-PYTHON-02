...
class General_Patient (object):
    ...
    def payment_status (self, cost_per_day, days_of_hospitalization):
        if self.insurance:
            print(f'The patient {self.first_name} {self.last_name} has nothing to pay \n')
            return (self.first_name, self.last_name, 0)
        else:
            f'The patient {self.first_name} {self.last_name} has to pay {cost_per_day*days_of_hospitalization} ron \n'
            return (self.first_name, self.last_name, cost_per_day*days_of_hospitalization)
...
class Diabetic_Patient (General_Patient):
    ...
    def special_placement (self):
        for medicine in self.medication:
            if medicine == 'insulin':
                print(f'Patient {self.first_name} {self.last_name} is to be placed in a special room \n')
                return True
        print(f'Patient {self.first_name} {self.last_name} can be placed in any room is available first \n')
        return False 
...
print (patient_4.special_placement())

print ('*' * 113)

patient_5.risk_message()
print (patient_5.special_placement())
